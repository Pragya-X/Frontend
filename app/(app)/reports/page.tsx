...
export default function ReportsPage() {
  ...
  return (
    <div className="space-y-4">
      <div>
        <h1 className="text-lg font-bold text-slate-700">Report Generation</h1>
        <p className="text-xs text-slate-400">Incident reports, daily intelligence briefings and zone reports as downloadable PDF</p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
        <Card>
          <CardBody className="flex items-center gap-4">
            ...
            <div className="flex-1">
              <p className="text-sm font-semibold text-slate-700">Daily Intelligence Report</p>
              <p className="text-[11px] text-slate-400">Hotspot summary with classifications, risk and alert counts for the last 24 hours.</p>
            </div>
        ...
        <Card>
          <CardBody className="flex items-center gap-4">
            ...
            <div className="flex-1">
              <p className="text-sm font-semibold text-slate-700">Industrial Zone Reports</p>
              <p className="text-[11px] text-slate-400">Per-zone risk, hotspot proximity and population exposure - open the Industrial Zones page and use the report button.</p>
            </div>
        ...
        <CardBody>
          ...
          {loading ? <Skeleton className="h-40" /> : error ? <ErrorState message={error} onRetry={load} /> : (
            <div className="space-y-1.5">
              {hotspots.length === 0 && <p className="py-6 text-center text-xs text-slate-400">No hotspots match.</p>}
              {hotspots.slice(0, 25).map((h) => (
                <div key={h.id} className="flex flex-wrap items-center gap-2 rounded-md border border-base-border/40 bg-base-raised/30 px-3 py-2">
                  ...
                  <span className="ml-auto text-[10px] text-slate-400">{h.state} / {h.district}</span>
