...
export function LiveFeed() {
  ...
  useEffect(() => {
    if (!latest) return;
    const meta = TYPE_META[latest.type] || { label: "System event", tone: "text-slate-600", icon: <Radar className="h-3.5 w-3.5" /> };
  ...
  return (
    <Card className="flex h-full flex-col">
      <CardHeader>
        ...
        <span className="text-[10px] uppercase tracking-wider text-slate-600">SSE stream</span>
      </CardHeader>
      <CardBody className="flex-1 overflow-y-auto p-3">
        ...
        <ul className="space-y-2">
          {items.map((it) => {
            const meta = TYPE_META[it.type] || { label: "System event", tone: "text-slate-600", icon: <Radar className="h-3.5 w-3.5" /> };
            return (
              <li key={it.id} className="animate-fadeIn rounded-md border border-base-border/50 bg-base-raised/40 px-2.5 py-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[11px] font-semibold text-slate-900">{it.label}</span>
                  <span className="ml-auto text-[9px] text-slate-600">{timeAgo(it.at.toISOString())}</span>
                </div>
                <p className="mt-0.5 line-clamp-2 pl-5 text-[10px] text-slate-500">{it.detail}</p>
              </li>
            );
          })}
        </ul>
